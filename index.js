...
function getAverageWordLength(array){
    let total = 0;
    for (let i = 0; i < array.length; i++) {
        let wordsInElement = array[i].split(' ');
        total = total + wordsInElement.length;
    }
...
function getRandomFlavors(array1, array2, array3, array4){
    ...
    mergedArray = mergedArray.concat(array4);

    for (let i = 0; i < 31; i++) {
        let randomFlavorIndex = Math.floor(Math.random() * mergedArray.length);
        randomFlavors.push(mergedArray[randomFlavorIndex]);
        mergedArray.splice(randomFlavorIndex, 1);
    }
